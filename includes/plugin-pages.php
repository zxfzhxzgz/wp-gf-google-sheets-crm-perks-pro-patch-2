  ...
  */
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'gravity-forms-googlesheets-crm') . '</a>' );
        }
        return $links;
   }
  ...
  /**
    ...
    public function add_form_settings_menu( $tabs, $form_id ) {
        ...
        $tabs[] = array( 'name' => $this->id, 'label' => __("Google Sheets", 'gravity-forms-googlesheets-crm') , 'query' => array( 'fid' => null) );
...
$this->push($entry,$form,'add_note');
  ...
  */
  ...
  $menus[] = array("name" => $this->id, "label" => __('Google Sheets','gravity-forms-googlesheets-crm'), "callback" =>  array($this, "mapping_page"), "permission" => $this->id.'_read_feeds');
  ...
  */
  public  function setup(){
      ...
      RGForms::add_settings_page(array('name' => $this->id,'tab_label' => __('Google Sheets','gravity-forms-googlesheets-crm'),"handler"=>array($this, "settings_page")));
  ...
  */
  ...
  _e('You do not have permissions to access this page','gravity-forms-googlesheets-crm');    
  return;
  }
...
public function get_menu_links($current_page=""){
  ...
  $logs_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview={$this->id}&tab=log&id={$_GET['id']}" );
  $feeds_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview={$this->id}&tab=feed&id={$_GET['id']}" );
...
foreach($map as $field_k=>$field_v){
  ...
  ?>
   <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('4. Select Google Sheet Tab.', 'gravity-forms-googlesheets-crm'); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_module" class="left_header"><?php _e("Google Sheets Tab", 'gravity-forms-googlesheets-crm'); ?>
 </label>
  </div>
  <div class="vx_col2">
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Google Sheets','gravity-forms-googlesheets-crm');
        return $plugins;
    }
  /**
...
$sel2_css=$base_url. 'css/select2.min.css';
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','gravity-forms-googlesheets-crm'); 
  return;
  }
  //
...
include_once(self::$path . "templates/feed-account.php");
  ...
  */
  ...
  array_push($form['fields'],array("id" => "date_created" , "label" => __("Entry Date", 'gravity-forms-googlesheets-crm')));
  array_push($form['fields'],array("id" => "ip" , "label" => __("User IP", 'gravity-forms-googlesheets-crm')));
  array_push($form['fields'],array("id" => "source_url" , "label" => __("Source Url", 'gravity-forms-googlesheets-crm')));
  array_push($form['fields'],array("id" => "form_title" , "label" => __("Form Title", 'gravity-forms-googlesheets-crm')));
