...
return array('tabs'=>$sheets,'fields'=>$fields);      
    ...
    return __('Sheet header is empty , please add first row  in sheet as column titles ','gravity-forms-googlesheets-crm');
}
}else{
...
// first cell of header row should be not empty  , other it will continue overwrting first row only
  $path=$this->url.'v4/spreadsheets/'.$object.'/values/'.urlencode("'".$meta['tab']."'!A:A").':append?valueInputOption='.$type.'&insertDataOption='.$row_type; //insertDataOption=INSERT_ROWS
  $post=array('values'=>array($fields),"majorDimension"=>"ROWS");
$google_res=$this->post_google_arr($path,"post",json_encode($post));
//var_dump($google_res,$fields,$path); die();
